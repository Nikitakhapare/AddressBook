...
public class CreateAddBook {
	...
	static void deleteContact() {
		if (counter>0) {
			System.out.println("Enter a Person First Name you want to delete ");
			String searchName=sc.next();
			for(int i=0; i<personList.size();i++) {
				if (personList.get(i).getFirstName().equals(searchName)) {
					personList.remove(i);
	...
	static void addMultipleContact() {
		System.out.println("Enter No ofPersion you want to add in Address Book ");
	...
	static void displayContact() {
		System.out.println("Contact Present in Address Book");
		for(int i=0;i<personList.size();i++) {
